...
** Last update Sat Nov 24 21:26:07 2012 louis-philippe baron
*/
...
char		*cpy_buffer(char *buffer, char *save)
{
  char		*str;
  int		i;
  int		k;
  ...
  if (save)
    while (save[i])
      i++;
  if ((str = malloc((i + READSIZE + 1) * sizeof(char))) == NULL)
    return (NULL);
  i = 0;
  k = 0;
  if (save)
...
char		*get_next_line(const int fd)
{
  ...
  if ((buffer = malloc((READSIZE + 1) * sizeof(char))) == NULL
    ...
    return (NULL);
  buffer[rd] = '\0';
  if ((save = cpy_buffer(buffer, save)) == NULL)
    return (NULL);
  while (check_n(buffer) == 0 && (rd = read(fd, buffer, READSIZE)) != 0)
    save = cpy_buffer(buffer, save);
  free(buffer);
  if (!(res = cpy_to_res(save)))
    {
      free(save);
      return (NULL);
    }
  if (!(save = save_end(res, save)))
    return (NULL);
  return (res);
}
